...
- **Navigation (Mobile Friendly)**:
  - 브라우저 **뒤로가기** 버튼으로 모달/줌 화면 닫기 지원
  - 결과 화면 뒤로가기 시 **재생성(API call) 방지** (History State Caching)
- **Advanced Generation Stability**:
  - `generate_time_change`, `multi_angle`, `pose` 기능의 **422 Error (Schema)** 및 **500 Error (Server)** 해결
  - 부분 성공(Partial Success) 지원: 여러 장 생성 중 일부가 실패해도 나머지는 정상 표시
  - 이미지 모델을 **`gemini-2.5-flash-image`**로 원복 + 에러 핸들링 강화

### 🐛 Fixed
- **Mobile Upload 500 Error**: 모바일 업로드 이미지(RGBA, Palette 등)를 RGB로 자동 변환하여 서버 에러 방지
- **Frontend Build Fix**: `ResultPage.tsx` 컴파일 오류(import mismatch) 해결
